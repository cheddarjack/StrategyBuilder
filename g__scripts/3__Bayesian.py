...
# Define P-Core PUs
P_CORE_PUS = list(range(16))  # P#cc0 to P#15
SUB_JOBS = 1
JOBS = 8
TRIALS = 1000
CONFIG_PATH = '/home/cheddarjackk/StrategyBuilder/c__config/config.yaml'
...
# Configure logging
logging.basicConfig(
    filename='e__output/optimization.log',
    level=logging.INFO,
...
def custom_callback(total_TRIALS):
    def callback(study, trial):
        ...
        if sessions in session_prefix_mapping and profit > 0 :
            ...
            print(f"Trial {trial.number} | sessions: {sessions} | {margin_str} | $$$: {profit_str} | adj: {adjustment_str} | trades: {trades} | score: {value} | [{duration_str}]")
    return callback
...
try:
    ...
    study = optuna.create_study(study_name=STUDY_NAME,storage=storage,direction='maximize',sampler=TPESampler(seed=42))
...
# cd /home/cheddarjackk/StrategyBuilder/output
